...
public enum Opus {
    ...
    /// Internet links pay a per-packet overhead the local ones do not, so they carry twice the audio per packet.
    public static let internetFrameMs = 20
    public static let internetFrameSamples = Int(sampleRate) * internetFrameMs / 1000
    ...
    static func opusFormat(frameSamples: Int) -> AudioStreamBasicDescription {
        AudioStreamBasicDescription(
...
public final class OpusEncoder: @unchecked Sendable {
    ...
    public init(bitrate: UInt32 = 24000, frameMs: Int = Opus.frameMs) throws {
        frameSamples = Int(Opus.sampleRate) * frameMs / 1000
        var input = Opus.pcmFormat
        var output = Opus.opusFormat(frameSamples: frameSamples)
    ...
    /// `pcm` must hold exactly one frame's samples for the frame size this encoder was built with.
    public func encode(_ pcm: UnsafeMutableBufferPointer<Float>) throws -> Data {
        precondition(pcm.count == frameSamples)
        pending = pcm
        consumed = false
...
public final class OpusDecoder: @unchecked Sendable {
    ...
    /// One converter decodes one frame size: fed a packet of the other size it returns zero frames,
    /// so a stream carrying 20 ms frames needs its own decoder.
    public init(frameMs: Int = Opus.frameMs) throws {
        frameSamples = Int(Opus.sampleRate) * frameMs / 1000
        var input = Opus.opusFormat(frameSamples: frameSamples)
    ...
    /// Returns how many samples the packet produced, at most this decoder's frame size.
    @discardableResult
    public func decode(_ packet: Data, into pcm: UnsafeMutableBufferPointer<Float>) throws -> Int {
        precondition(pcm.count >= frameSamples)
        self.packet = packet
        consumed = false
        var frames = UInt32(pcm.count)
        var list = AudioBufferList(mNumberBuffers: 1, mBuffers: AudioBuffer(
